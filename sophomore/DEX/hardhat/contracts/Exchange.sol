...
contract Exchange is ERC20 {
	...
	constructor(address _CryptoDevtoken, string memory _name, string memory _symb) ERC20(_name, _symb) {
		require(_CryptoDevtoken != address(0), "Token address passed is a null address");
		cryptoDevTokenAddress = _CryptoDevtoken;
	}

	/**
	 *@dev Retunrs the amount of CDT tokens
	 */
    ...
    */
	...
	function getAmountOfTokens(uint256 inputAmount, uint256 inputReserve, uint256 outputReserve) public pure returns (uint256) {
		require(inputReserve > 0 && outputReserve > 0, "no more liquidity");

		uint inputAmountWithFees = (99 * inputAmount) / 100;

		// x * y = (x +dx) * (y -dy)
		uint tokenToGive = (outputReserve * inputAmountWithFees) / (inputReserve + inputAmountWithFees);
